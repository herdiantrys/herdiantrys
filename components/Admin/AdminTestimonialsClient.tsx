...
import { Search, Plus, ArrowUpDown, ArrowUp, ArrowDown, CheckSquare, Square, Trash2, X, User } from "lucide-react";
import { useRouter } from "next/navigation";
import { bulkDeleteTestimonials } from "@/lib/actions/testimonial.actions";
...
export default function AdminTestimonialsClient({ testimonials }: { testimonials: any[] }) {
    ...
    const toggleSelectAll = () => {
        if (selectedIds.length > 0) {
            ...
            setSelectedIds(filteredAndSortedTestimonials.map(t => t.id));
        }
    };
    ...
    const filteredAndSortedTestimonials = useMemo(() => {
        ...
        return result;
    }, [testimonials, sortConfig, searchQuery]);
    ...
    return (
        <div>
                        ...
                        <tr>
                            <th className="px-6 py-4 w-10">
                                <button onClick={toggleSelectAll} className="flex items-center text-gray-400 hover:text-white transition-colors">
                                    {selectedIds.length === paginatedTestimonials.length && paginatedTestimonials.length > 0 ? <CheckSquare size={18} className="text-teal-400" /> : <Square size={18} />}
                                </button>
                            </th>
                            <SortHeader label="Author" columnKey="name" />
                            <SortHeader label="Role" columnKey="role" />
                            <SortHeader label="Content" columnKey="content" />
                            ...
                            paginatedTestimonials.map((testimonial) => (
                                <tr key={testimonial.id} className={`hover:bg-gray-50 dark:hover:bg-white/5 transition-colors group ${selectedIds.includes(testimonial.id) ? "bg-[var(--site-accent)]/5" : ""}`}>
                                    ...
                                    <td className="px-6 py-4 flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-700/50 relative flex-shrink-0 border border-white/5">
                                            {testimonial.photo ? (
                                                <Image
                                                    src={resolveAssetUrl(testimonial.photo)}
            ...
            <DeleteConfirmationModal
                ...
                description={`Are you sure you want to delete ${selectedIds.length} selected testimonials? This action cannot be undone.`}
            />
